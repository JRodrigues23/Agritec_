...
public class AlimentosDao {
	...
	stmt.setString(1, alimentos.getDataDeColheita());
	stmt.setString(2, alimentos.getDataDeValidade());
	stmt.setString(3, alimentos.getTipo());
	...
	//CONSULTA 
		public Alimentos colsultarAlimentosId(int idaAlimentos) throws ClassNotFoundException, SQLException{
			Connection connection = ConexaoMySQL.getConexaoMySQL();
			String sql = "SELECT `idAlimentos`, `nome`, `email`,  `dataDeValidade`, `dataDeColheita`, `tipo` FROM `alimentos` WHERE idAlimentos=?";

			PreparedStatement stmt = connection.prepareStatement(sql);
			stmt.setInt(1,idAimentos);
			...
			if(resultSet.next()) {
				...
				String nome = resultSet.getString(2);
				alimentos.setNome(nome);
				String dataDeColheita = resultSet.getString(3);
				alimentos.setDataDeColheita(dataDeColheita);
				
				alimentos.setDataDeValidade(resultSet.getNString(4));
				alimentos.setTipo(resultSet.getNString(5));
